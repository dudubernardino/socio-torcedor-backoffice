...
export const OrgForm = ({ org = {}, disabled, setDisabled, onSubmit }) => {
  const {
    register,
    handleSubmit,
    control,
  ...
  const orgTypes = process.env.NEXT_PUBLIC_ORG_TYPES
    ? process.env.NEXT_PUBLIC_ORG_TYPES.split(',').map((item) => ({
        value: item,
        label: item,
      }))
    : []
        ...
        <div className="rounded-t bg-white mb-0 px-10 py-6">
          <div className="text-center flex justify-between">
            <h6 className="text-gray-700 text-xl font-bold">
              {org?.tradeName || 'Registrar nova organização'}
            </h6>
            <div>
              {disabled ? (
                <>
